...
int main(int argc, char* argv[])
{
    try
    {
        ...
        while (std::cin.getline(line, chat_message::max_body_length + 1))
        {
            ...
            to_dealer["from"] = { {"name": "Chandler"}, {"id": "1001674404"} }; //The current player.
            ...
            if (to_dealer["event"] == "check")
                to_dealer["chat"] = to_dealer.from.name " has checked to the next player.";
            if else (to_dealer["event"] == "bet")
                to_dealer["chat"] = to_dealer.from.name " has started the bet.";
            if else (to_dealer["event"] == "raise")
                to_dealer["chat"] = to_dealer.from.name " has raised the bet.";
            if else (to_dealer["event"] == "fold")
                to_dealer["chat"] = to_dealer.from.name " has folded and is out of the game.";
            if else (to_dealer["event"] == "call")
                to_dealer["chat"] = to_dealer.from.name " has called the bet.";
